...
def home():
    ...
    if query:
        ...
        daftar_kematian_records = DaftarKematian.query.filter(
            (DaftarKematian.deceased_name.contains(query)) |
            (DaftarKematian.stone_number.contains(query)) |
            (DaftarKematian.heir_name.contains(query)) |
            (DaftarKematian.heir_contact.contains(query))
        ).all()
